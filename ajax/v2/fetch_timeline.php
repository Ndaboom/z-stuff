...
if(isset($_POST["limit"], $_POST["start"]))
  {
        $q=$db->prepare("SELECT * FROM microposts WHERE user_id=? ORDER BY -created_at LIMIT ".$_POST["start"].", ".$_POST["limit"]."");
            ...
            foreach ($result as $row)
               {
                ...
                if(already_in_cart($_SESSION['user_id'],$row->id)){
                    $order_btn = '
                    <a href="" data-pr_i="'.$row->id.'" data-object_price="'.$row->additionnal_info.'" data-action="Acheter" data-designation="'.$row->title.'" data-image="'.$row->urlMedia.'" class="bg-blue-600 flex flex-1 font-semibold h-10 items-center justify-center px-4 rounded-md text-white add_product">
                    Added
                    </a>
                    ';
                }else{
                    $order_btn = ' <a href="" data-pr_i="'.$row->id.'" data-object_price="'.$row->additionnal_info.'" data-action="Acheter" data-designation="'.$row->title.'" data-image="'.$row->urlMedia.'" class="bg-blue-600 flex flex-1 font-semibold h-10 items-center justify-center px-4 rounded-md text-white add_product">
                    Add to Cart 
                    </a>';
                }
                 ...
                 <div class="post_box bg-white shadow border border-gray-100 rounded-lg dark:bg-gray-900 lg:mx-0 uk-scrollspy-inview uk-animation-slide-bottom" id="post_box'.$row->id.'">
                          ...
                          <div class="text-2xl font-semibold pt-2"> '.$row->title.'</div>
                                ...
                                <div class="grid grid-cols-2 gap-4 mb-5">
                                    <a href="messages.php?n='.$current_user->name.'&i='.$row->user_id.'" class="bg-gray-200 flex flex-1 font-semibold h-10 items-center justify-center px-4 rounded-md">
                                        Contact seller
                                    </a>
                                   '.$order_btn.'
                                </div>
